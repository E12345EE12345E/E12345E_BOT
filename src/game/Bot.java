...
public class Bot {
    ...
    public static String compute3(Game game, int depth, int cullAmount) { // wide search with combo finding
        double minscore = depth>4?5:4;
        MoveScore topCombo = recurse3(game, new ArrayList<MoveScore>(), 0, minscore);
        if (topCombo.score - Math.min(game.combo, minscore-1) <= 0) {
            return topCombo.movelist;
        }
        return recurse2(game, new ArrayList<MoveScore>(), depth, 1.1, cullAmount).get(0).movelist;
    }
    ...
    public static List<MoveScore> recurse2(Game game, List<MoveScore> moves, int depth, double scoreDecay, int cullAmount) {
        ...
        for (int i=0; i<moves.size(); i++) {
            ...
            List<MoveScore> secondaryList = moveTree(newGame, newGame.board.countHoles() > 0);
    ...
    public static MoveScore recurse3(Game game, List<MoveScore> moves, int depth, double minscore) {
        List<MoveScore> moveList = new ArrayList<MoveScore>();
        if (moves.size() == 0) moves.add(new MoveScore("",minscore));
        ...
        for (int i=0; i<moves.size(); i++) {
            ...
            List<MoveScore> secondaryList = moveTree(newGame, newGame.board.countHoles() > 0);
            for (int j=0; j<secondaryList.size(); j++) {
                Game newGame2 = newGame.getGame();
                newGame2.doMovement(secondaryList.get(j).movelist);
                if (newGame2.combo > 0) {
                    double bonus = newGame2.mostRecentLinesSent>8?newGame2.mostRecentLinesSent:(newGame2.mostRecentLinesSent>5?newGame2.mostRecentLinesSent/3.0:0);
                    moveList.add(new MoveScore(moves.get(i).movelist + secondaryList.get(j).movelist, moves.get(i).score - bonus - (newGame2.combo>2?1:0.5)));
                    hasCombo = true;
                }
            }
        }
        if (depth < minscore && hasCombo) {
            int cullVal = Consts.BoardWidth>4?80:800;
            if (moveList.size() > cullVal) {
                MoveScore.sort(moveList);
                List<MoveScore> newMoveList = new ArrayList<MoveScore>();
                for (int i=0; i<cullVal; i++) {
                    newMoveList.add(moveList.get(i));
                }
                return recurse3(game, newMoveList, depth+1, minscore);
            } else {
                return recurse3(game, moveList, depth+1, minscore);
            }
        } else {
            MoveScore.sort(moves);
            return moves.get(0);
        }
    }
